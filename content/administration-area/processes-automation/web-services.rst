...
OTOBO supports this requirement by the *Generic Interface*.
It empowers the administrator to create a web service for a specific task without scripting language knowledge.
...
In its configuration, the :term:`operation` or :term:`invoker` determine the direction of communication,
and the :term:`mapping` and :term:`transport` take care of how the data is received and interpreted.

By configuration, you can define what actions the web service can perform internally (operation),
what actions the OTOBO request can perform on a remote system (invokers), how data is converted from one system to the other (mapping),
and which protocol is used for communication (transport).
...
Validity
   ...
   Resources can only be used in OTOBO if this field is set to *valid*.
...
- optionally, a set of ``scope`` parameters to use.
  When using OIDC this likely will include 'openid'
...
First, configure an OIDC Provider Profile:
...
If you are already using OIDC for Login, some values will be pre-populated.
You have to provide a valid, unique Profile name and also provide the OAuth2 application client_secret value,
which is not pre-populated for security reasons.
Usually you can keep all the defaults, especially for the optional SSL and Misc.
settings.

Now that we have an OIDC Provider Profile, we can set up a Functional Account.
There can be more than one Functional Account per Profile.
...
Choose the ``GrantType`` as instructed by your system administrator, and provide the ``Scope`` parameter as required.
``Scope`` will often contain ``openid`` and ``offline_access``, other common values are ``email`` and ``profile``.
...
In case your refresh_tokens expire sooner than 60 minutes, you'll need to adapt.

The only parameters to tune are the Cron Schedule expression itself, and the ``Interval`` parameter.
The cron expression can be any valid cron expression, examples include:
...
For the ``Interval`` parameter set it to the time period between two scheduled task runs, in seconds.
Running the cronjob once per hour gives you 60 \* 60 = 3600 seconds between each run, and thus the ``Interval`` parameter should be set to ``3600``.
...
This console command can also be used for debugging OIDC/OAuth2 problems:

::

   bin/otobo.Console.pl Admin::OAuth2::ImportUser --help
